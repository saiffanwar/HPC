...
int main(int argc, char* argv[])
{
  ...
  subgrid = (float*)malloc(sizeof(float) * (local_ncols+2) * (local_nrows+2));
  tmp_subgrid = (float*)malloc(sizeof(float) * (local_ncols+2) * (local_nrows+2));
  ...
  for(int i = 0; i<size; i++){
    ...
    displs[i] = i * (local_nrows+2) * local_nrows;
  }

  MPI_Scatterv(image+width, sendcounts, displs, MPI_FLOAT, subgrid + (local_nrows+2), (local_nrows+2)*local_nrows, MPI_FLOAT, 0, MPI_COMM_WORLD);


    // Call the stencil kernel
    double tic = wtime();
    for (int t = 0; t < niters; ++t) {
      halo_exchange(local_ncols, local_nrows, (local_nrows+2), (local_ncols+2), right, left, subgrid, tmp_subgrid);
      stencil(local_ncols, local_nrows, (local_nrows+2), (local_ncols+2), right, left, subgrid, tmp_subgrid);
      halo_exchange(local_ncols, local_nrows, (local_nrows+2), (local_ncols+2), right, left, tmp_subgrid, subgrid);
      stencil(local_ncols, local_nrows, (local_nrows+2), (local_ncols+2), right, left, tmp_subgrid, subgrid);
    }
    double toc = wtime() - tic;
    double time;

    MPI_Gatherv(subgrid+(local_nrows+2), (local_nrows+2)*local_nrows, MPI_FLOAT, image+width, sendcounts, displs, MPI_FLOAT, 0, MPI_COMM_WORLD);
